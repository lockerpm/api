...
class AbstractRelaySubdomainORM(models.Model):
    subdomain = models.CharField(max_length=128, db_index=True)
    created_time = models.FloatField()
    is_deleted = models.BooleanField(default=False)
    user = models.ForeignKey(
        locker_server_settings.LS_USER_MODEL, on_delete=models.CASCADE, related_name="relay_subdomains"
    )
    domain = models.ForeignKey(
        locker_server_settings.LS_RELAY_DOMAIN_MODEL, on_delete=models.CASCADE, related_name="relay_subdomains"
    )
