...
def get_school_names(domain: Parts) -> List[str]:
    ...
    base_url = "https://raw.githubusercontent.com/JetBrains/swot/master/lib/domains"
    url = base_url

    for part in domain:
        url += f"/{part}"
        text_form_url = f"{url}.txt"

        try:
            res = requester(method="GET", url=text_form_url)
        except (requests.exceptions.RequestException, requests.exceptions.ConnectTimeout):
            continue
        if res.status_code != 200:
            continue
        schools = res.text
        return [name.strip() for name in schools.split("\n") if name.strip() != ""]
    return []
