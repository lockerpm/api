...
class WalletPaymentMethod(PaymentMethod):
    ...
    def cancel_recurring_subscription(self, **kwargs):
        ...
        if "cancel_at_period_end" not in kwargs:
            cancel_at_period_end = True if self.user_plan.cancel_at_period_end is False else False
        else:
            cancel_at_period_end = kwargs.get("cancel_at_period_end")
        new_user_plan = user_service.update_user_plan_by_id(user_plan_id=self.user_plan.user.user_id, user_plan_update_data={
            "cancel_at_period_end": cancel_at_period_end
        })
        if new_user_plan.cancel_at_period_end is True:
            return new_user_plan.end_period
        return None
    ...
    def cancel_immediately_recurring_subscription(self, **kwargs):
        ...
        user_service.update_plan(
            user_id=self.user_plan.user.user_id, plan_type_alias=PLAN_TYPE_PM_FREE, scope=self.scope
        )
